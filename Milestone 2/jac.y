...
Program:  CLASS ID OBRACE Subprogram CBRACE {if (flag_tree_builder==1){$$ = New_program(Terminals("Id",$2),$4);Program=$$;}}
...
FieldDecl:  PUBLIC STATIC Type ID SubFieldDecl SEMI {if (flag_tree_builder==1){ $$ = new_Decl("FieldDecl", $3 , Terminals("Id",$4) , $5);auxVarDecl=NULL;}}
         |  error SEMI                              {if (flag_tree_builder==1){ $$ = NULL;cancel_tree();flag_tree_builder=0;}}

SubFieldDecl: SubFieldDecl COMMA ID                 {if (flag_tree_builder==1) $$ = new_Decl2(Terminals("Id",$3));}
            | %empty                                {if (flag_tree_builder==1) $$ = NULL;}

MethodDecl: PUBLIC STATIC MethodHeader MethodBody   {if (flag_tree_builder==1) $$ = new_MethodDecl($3,$4);}

MethodHeader: VOID ID OCURV SubMethodHeaderBI CCURV {if (flag_tree_builder==1) $$ = new_MethodHeader(Terminals("Void",NULL),Terminals("Id",$2),$4);}
            | Type ID OCURV SubMethodHeaderBI CCURV {if (flag_tree_builder==1) $$ = new_MethodHeader($1,Terminals("Id",$2),$4);}
...
FormalParams: Type ID SubFormalParams        {if (flag_tree_builder==1){ $$ = new_ParamDecl($1,Terminals("Id",$2),$3);auxFormalParams=NULL;}}
            | STRING OSQUARE CSQUARE ID      {if (flag_tree_builder==1)  $$ = new_ParamDecl(Terminals("StringArray",NULL),Terminals("Id",$4),NULL);}

SubFormalParams: SubFormalParams COMMA Type ID              {if (flag_tree_builder==1) $$ = aux_ParamDecl($3, Terminals("Id",$4));}
               | %empty                                     {if (flag_tree_builder==1) $$ = NULL;}

VarDecl: Type ID SubFieldDecl SEMI          {if (flag_tree_builder==1){ $$ = new_Decl("VarDecl", $1 , Terminals("Id",$2) , $3);auxVarDecl=NULL;}}
...
Statement: OBRACE SubStatement CBRACE                    {if (flag_tree_builder==1){ $$ = Statement_Block($2);auxStatement=NULL;}}
         | IF OCURV Expr CCURV Statement ELSE Statement  {if (flag_tree_builder==1) $$ = Statement_If($3,$5,$7);}
         | IF OCURV Expr CCURV Statement %prec ELSE      {if (flag_tree_builder==1) $$ = Statement_If($3,$5,NULL);}
         | WHILE OCURV Expr CCURV Statement              {if (flag_tree_builder==1) $$ = Statements_2("While",$3,$5);}
         | DO Statement WHILE OCURV Expr CCURV SEMI      {if (flag_tree_builder==1) $$ = Statements_2("DoWhile",$2,$5);}
         | PRINT OCURV SubStatementOpt CCURV SEMI        {if (flag_tree_builder==1) $$ = Statement_Print($3);}
         | SubStatementBI2 SEMI                          {if (flag_tree_builder==1) $$ = $1;}
         | RETURN SEMI                                   {if (flag_tree_builder==1) $$ = Statement_Return(NULL);}
         | RETURN Expr SEMI                              {if (flag_tree_builder==1) $$ = Statement_Return($2);}
         | error SEMI                                    {if (flag_tree_builder==1){ $$ = NULL;cancel_tree();flag_tree_builder=0;}}
...
Assignment: ID ASSIGN Expr                  {if (flag_tree_builder==1) $$=Operators_2("Assign",Terminals("Id",$1),$3);}

MethodInvocation: ID OCURV CCURV                              {if (flag_tree_builder==1)  $$ = Operator_Call(Terminals("Id",$1), NULL , NULL);}
                | ID OCURV Expr SubMethodInvocationOpt CCURV  {if (flag_tree_builder==1){ $$ = Operator_Call(Terminals("Id",$1), $3 , $4);}}
...
ParseArgs: PARSEINT OCURV ID OSQUARE Expr CSQUARE CCURV   {if (flag_tree_builder==1) $$=Operators_2("ParseArgs",Terminals("Id",$3),$5);}
...
ExprAux: MethodInvocation                       {if (flag_tree_builder==1) $$ = $1;}
        ...
        | PLUS ExprAux   %prec NOT              {if (flag_tree_builder==1) $$ = Operators_1("Plus",$2);}
        | MINUS ExprAux  %prec NOT              {if (flag_tree_builder==1) $$ = Operators_1("Minus",$2);}
        | NOT ExprAux                           {if (flag_tree_builder==1) $$ = Operators_1("Not",$2);}
        | ID                                    {if (flag_tree_builder==1) $$ = Terminals("Id",$1);}
        | ID DOTLENGTH                          {if (flag_tree_builder==1) $$ = Operators_1("Length",Terminals("Id",$1));}
        | OCURV Expr CCURV                      {if (flag_tree_builder==1) $$ = $2;}
        | BOOLLIT                               {if (flag_tree_builder==1) $$ = Terminals("BoolLit",$1);}
        | DECLIT                                {if (flag_tree_builder==1) $$ = Terminals("DecLit",$1);}
        | REALLIT                               {if (flag_tree_builder==1) $$ = Terminals("RealLit",$1);}
        | OCURV error CCURV                     {if (flag_tree_builder==1){ $$ = NULL;cancel_tree();flag_tree_builder=0;}}

SubExpr: ExprAux AND ExprAux                      {if (flag_tree_builder==1) $$=Operators_2("And",$1,$3);}
       | ExprAux OR ExprAux                       {if (flag_tree_builder==1) $$=Operators_2("Or",$1,$3);}

SubExpr2: ExprAux EQ ExprAux                      {if (flag_tree_builder==1) $$=Operators_2("Eq",$1,$3);}
        | ExprAux GEQ ExprAux                     {if (flag_tree_builder==1) $$=Operators_2("Geq",$1,$3);}
        | ExprAux GT ExprAux                      {if (flag_tree_builder==1) $$=Operators_2("Gt",$1,$3);}
        | ExprAux LEQ ExprAux                     {if (flag_tree_builder==1) $$=Operators_2("Leq",$1,$3);}
        | ExprAux LT ExprAux                      {if (flag_tree_builder==1) $$=Operators_2("Lt",$1,$3);}
        | ExprAux NEQ ExprAux                     {if (flag_tree_builder==1) $$=Operators_2("Neq",$1,$3);}

SubExpr3: ExprAux PLUS ExprAux                    {if (flag_tree_builder==1) $$=Operators_2("Add",$1,$3);}
        | ExprAux MINUS ExprAux                   {if (flag_tree_builder==1) $$=Operators_2("Sub",$1,$3);}
        | ExprAux STAR ExprAux                    {if (flag_tree_builder==1) $$=Operators_2("Mul",$1,$3);}
        | ExprAux DIV ExprAux                     {if (flag_tree_builder==1) $$=Operators_2("Div",$1,$3);}
        | ExprAux MOD ExprAux                     {if (flag_tree_builder==1) $$=Operators_2("Mod",$1,$3);}

%%

/*Creating ROOT Program*/
